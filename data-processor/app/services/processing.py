...
class ProcessingService:
        ...
        for country in snapshot.countries:
            for city in country.cities:
                for station in city.places:
                    ...
                    self.redis_repo.update_station_occupancy(station.uid, current_bike_numbers)
                    
                    for bike in station.bike_list:
                        self._detect_and_record_movement(bike.number, station.uid, processing_time)
    ...
    def _detect_and_record_movement(self, bike_number: str, current_station_uid: int, current_timestamp: float):
        ...
        if last_state and last_state.station_uid != current_station_uid:
            logger.info(f"Movement detected for bike {bike_number}: {last_state.station_uid} -> {current_station_uid}")
            ...
            if start_station and end_station:
                distance = self._haversine_distance(start_station.lat, start_station.lng, end_station.lat, end_station.lng)
            
            self.movement_repo.create({
                "bike_number": bike_number,
                "start_station_uid": last_state.station_uid,
                "end_station_uid": current_station_uid,
                "start_time": datetime.fromtimestamp(last_state.timestamp, tz=timezone.utc),
                "end_time": datetime.fromtimestamp(current_timestamp, tz=timezone.utc),
                "distance_km": distance
            })
        
        self.redis_repo.set_bike_state(bike_number, {"station_uid": current_station_uid, "timestamp": current_timestamp})
